(() => {
  ...
  function niceTimeTickSec(spanSec) {
    // Aim ~7 ticks.
    const target = spanSec / 7;
  ...
  function draw(data) {
    ...
    // Grid + ticks
    ctx.font = `${Math.max(11, Math.floor(W / 90))}px ui-sans-serif, system-ui`;
    ctx.fillStyle = fg;
    ...
    // X ticks (range-aware)
    const tickSec = niceTimeTickSec(spanSec);
    ...
    // Axis labels
    const axisFontPx = Math.max(12, Math.floor(W / 80));
    ctx.fillStyle = fg;
